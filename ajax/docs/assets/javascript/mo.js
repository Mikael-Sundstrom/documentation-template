...
GodLikeMouse http://www.godlikemouse.com/glm-ajax/
http://www.godlikemouse.com/glm-ajax/index.html
*/
...
MO.DOM.isInternetExplorer = (navigator.userAgent.indexOf("MSIE") >= 0);
MO.DOM.isMozilla = (navigator.userAgent.indexOf("Gecko") >= 0);
MO.DOM.isOpera = (navigator.userAgent.indexOf("Opera") >= 0);
MO.DOM.isSafari = (navigator.userAgent.indexOf("Safari") >= 0);
MO.Collection.Map = function () {
	var len = 0;
	var keys = new Array();
...
MO.AJAX = function () {
	var nameSpace = "http://multiplayers.online/";
	var map = new MO.Collection.Map();
	var ajaxObject = () => {
		try {
	...
	this.callService = function (serviceUrl, soapMethod, callbackFunction) {
		...
		var ao = ajaxObject();
		if (!ao.encode) {
			if (serviceUrl.indexOf("http://") < 0) serviceUrl = "http://" + serviceUrl;
			serviceUrl += "?WSDL";
			...
			ao.onreadystatechange = function () {
				if (ao.readyState == 4) {
					try {
						...
						if (!response) response = ao.responseXML.getElementsByTagName(soapMethod + "Response")[0];
						if (!response) {
							callServiceError("WebService does not contain a Result/Response node");
							return;
						}
						if (response.textContent) callbackFunction(response.textContent);
						else if (response.text) callbackFunction(response.text);
					} catch (ex) {
						callServiceError(ex);
					}
				}
			...
			if (serviceUrl.indexOf("http://") < 0) serviceUrl = document.location + serviceUrl;
			...
			try {
				ao.asyncInvoke(function (resp, call, status) {
					if (resp.fault) return callServiceError(resp.fault);
					if (!resp.body) {
